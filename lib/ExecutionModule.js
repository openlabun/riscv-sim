...
export function intructionExecution(descInst, regs, mem) {
  ...
  switch (opcode) {
    ...
    case "0110111": // lui ✔?  TIPO-U -> | imm[31:12] | rd      | opcode   |
      ...
      break
    case "0010111": // aupic ✔?
      Storing(regs, rd, intToBin32(ADD_binToInt(regs["PC"], SLL_bin(intToBin32(Number.parseInt(imm, 2)), "1100"))))
      break
    ...
    case "1100011": //TIPO-B -> | imm[12|10:5] | rs2  | rs1  | funct3 | imm[4:1|11] | opcode |
      ...
      dir = intToBin32(ADD_binToInt(intToBin32(Number.parseInt(dir, 2) * 2), regs["PC"]))
    ...
    case "0000011": //TIPO-I LOAD -> | imm[11:0] | rs1  | funct3 | rd   | opcode |
      ...
      if (Number.parseInt(memdirL, 2) % 4 == 0) {
        if (memdirL in mem) {
          switch (funct3) {
            case "000": // lb ✔?
            ...
            case "010": // lw ✔?
              ...
              break
            case "100": // lbu ✔?
              Storing(regs, rd, intToBin32(Number.parseInt(mem[memdirL].slice(-8), 2)))
              break
            case "101": // lhu ✔?
              Storing(regs, rd, intToBin32(Number.parseInt(mem[memdirL].slice(-16), 2)))
              break
          }
        } else {
    ...
    case "0100011": //TIPO-S -> | imm[11:5] | rs2  | rs1  | funct3 | imm[4:0] | opcode |
      ...
      if (Number.parseInt(memdirS, 2) % 4 == 0) {
        switch (funct3) {
          case "000": // sb ✔?
            Storing(mem, memdirS, intToBin32(Number.parseInt(regs[rs2].slice(-8), 2)))
            break
          case "001": //sh ✔?
            Storing(mem, memdirS, intToBin32(Number.parseInt(regs[rs2].slice(-16), 2)))
            break
          case "010": //sw ✔?
